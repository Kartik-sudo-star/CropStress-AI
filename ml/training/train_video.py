"""
Training script for Video Deepfake Detector
"""
...
from ml.models.video_detector import DeepfakeVideoDetector, create_video_detector, save_video_detector
...
class DeepfakeVideoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # Keep only rows that carry a video path (manifests are shared
        # across modalities: image / video / audio rows coexist)
        self.df = self.df.dropna(subset=["video_path"])
        self.df = self.df[self.df["video_path"].astype(str).str.len() > 0]
        self.df = self.df.dropna(subset=["label_idx"])
        self.df["label_idx"] = self.df["label_idx"].astype(int)
        
        logger.info(f"Loaded {len(self.df)} videos from {manifest_path}")
    ...
    def __getitem__(self, idx):
        ...
        try:
            video_tensor, _, _ = self.preprocessor.preprocess(
                video_path,
            ...
            # Return dummy tensor [T, C, H, W]
            T = self.preprocessor.frame_sampler.num_frames
            C, H, W = 3, *self.preprocessor.target_size
            video_tensor = torch.zeros(T, C, H, W)
        
        return video_tensor, label


def create_data_loaders(config, preprocessor, batch_size, num_workers=4):
    paths = config["paths"]
...
def main():
    ...
    if args.epochs:
        config["video_detector"]["training"]["epochs"] = args.epochs
    
    device = torch.device(config["general"]["device"])
    logger.add(config["paths"]["logs_dir"] + "/train_video.log", rotation="10 MB")
    ...
    # Create preprocessor
    preprocessor = create_video_preprocessor_from_config(config)
    preprocessor.save(os.path.join(config["paths"]["models_dir"], "video_detector", "preprocessor.pkl"))
    
    # Create data loaders
    train_loader, val_loader, test_loader = create_data_loaders(
        config,
        preprocessor,
        config["video_detector"]["training"]["batch_size"],
        config["general"]["num_workers"]
    )
    
    # Create model
    model = create_video_detector(config).to(device)
    logger.info(f"Model: {model.frame_backbone_name}, params: {sum(p.numel() for p in model.parameters())}")
    
    # Loss
    training_cfg = config["video_detector"]["training"]
    ...
    model_dir = Path(config["paths"]["models_dir"]) / "video_detector"
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_auc > best_val_auc:
            ...
            early_stopping_counter = 0
            save_video_detector(model, str(model_dir / "best_model.pth"), optimizer=optimizer, epoch=epoch, metrics={"val_auc": val_auc, **val_metrics}, config=config)
            logger.info(f"Saved best model (AUC: {val_auc:.4f})")
        else:
            early_stopping_counter += 1
        
        save_video_detector(model, str(model_dir / f"checkpoint_epoch_{epoch}.pth"), optimizer=optimizer, epoch=epoch, metrics={"val_auc": val_auc, **val_metrics}, config=config)
